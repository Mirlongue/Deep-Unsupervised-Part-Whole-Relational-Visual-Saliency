...
from utils import *
from Par_CRF import apply_dcrf_par, save_single_map
...
def train_round(args, target_dirs, output_dir_it, discretization_threshold, MapsOut = False):
    ...
    if args.pretrained:
        ...
        for name, param in load_dict.items():
            #print(name)
            name=name[5:]
            t_name='base.0.layer'+name
            if(t_name  in own_dict):
                #print(t_name)
                own_dict[t_name].copy_(param)


    model = torch.nn.DataParallel(single_model).cuda()
    ####summary(model,(3,352,352))

    # Data loading code#os.environ['CUDA_VISIBLE_DEVICES'] = '3'
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=batch_size, shuffle=True, num_workers=0,###num_workers,
    ...
    if not MapsOut:
        val_loader = torch.utils.data.DataLoader(
            ...
            batch_size=1, shuffle=False, num_workers=0,###num_workers,
            pin_memory=False, drop_last=True
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) %20==0 or epoch==args.epochs:####args.checkpoint_freq
            torch.save({
                'epoch': epoch + 1,
                'arch': args.arch,
...
def test_saliency(args):
    ...
    if not args.resume:
        args.resume = join(args.root_dir + args.weight_path)####
    start_epoch = 0
    if args.resume:
        if os.path.isfile(args.resume):
...
def train_unsupervised(args):
    ...
    #####
    target_dirs_refined.append(join(args.root_dir,'refined_nocrf_3/MC/'))
    target_dirs_refined.append(join(args.root_dir,'refined_nocrf_3/HS/'))
    target_dirs_refined.append(join(args.root_dir,'refined_nocrf_3/DSR/'))
    target_dirs_refined.append(join(args.root_dir,'refined_nocrf_3/RBD/'))
    #target_dirs_refined.append(join(args.root_dir,'Doc/Phase_I_Refined_Maps/MC/Iteration_3/MVAMaps/'))
    #target_dirs_refined.append(join(args.root_dir,'Doc/Phase_I_Refined_Maps/HS/Iteration_3/MVAMaps/'))
    #target_dirs_refined.append(join(args.root_dir,'Doc/Phase_I_Refined_Maps/DSR/Iteration_3/MVAMaps/'))
    #target_dirs_refined.append(join(args.root_dir,'Doc/Phase_I_Refined_Maps/RBD/Iteration_3/MVAMaps/'))
...
### train with update
def train_update(train_loader, model, optimizer, epoch, doc_directory, args, discretization_threshold,
          refined_labels_directory=None, iter_size=5,
          print_freq=10,  mc_mva=None,hs_mva=None,dsr_mva=None,rbd_mva=None, image2indx=None):
    ...
    for i, Data in tqdm(enumerate(train_loader)):
        ...
        #batch_data.discretize_pseudolabels(Disc_Thr)
        #### core
        batch_data.compute_loss_update(mc_mva[image2indx(batch_data.names)],hs_mva[image2indx(batch_data.names)],\
                                       dsr_mva[image2indx(batch_data.names)],rbd_mva[image2indx(batch_data.names)],args.batch_size,beta=args.beta,epoch=epoch,Disc_Thr=Disc_Thr)
        
        if(epoch>=150):
            mc_mva[image2indx(batch_data.names)] = batch_data.a.cpu()
            hs_mva[image2indx(batch_data.names)] = batch_data.b.cpu()
            dsr_mva[image2indx(batch_data.names)] = batch_data.c.cpu()
            rbd_mva[image2indx(batch_data.names)] = batch_data.d.cpu()
        ...
        #mc_mva[image2indx(batch_data.names)] = a####.cpu()
        #hs_mva[image2indx(batch_data.names)] = b
        #dsr_mva[image2indx(batch_data.names)] = c
        #rbd_mva[image2indx(batch_data.names)] = d
    ...
    return losses.avg, mc_mva,hs_mva,dsr_mva,rbd_mva


### init with label
def init_mva_preds_update(args, data_loader):

    images_names = data_loader.dataset.image_list
    n_samples = len(images_names)
    size = args.crop_size

    mc_mva = torch.zeros((n_samples, size, size))
    hs_mva = torch.zeros((n_samples, size, size))
    dsr_mva = torch.zeros((n_samples, size, size))
    rbd_mva = torch.zeros((n_samples, size, size))
    ...
    for i, Data in tqdm(enumerate(data_loader)):
        ...
        batch_data.check_dimension()
        batch_data.normalize_labels()
        mc_mva[image2indx(batch_data.names)] = batch_data.pseudolabels[0]
        hs_mva[image2indx(batch_data.names)] = batch_data.pseudolabels[1]
        dsr_mva[image2indx(batch_data.names)] = batch_data.pseudolabels[2]
        rbd_mva[image2indx(batch_data.names)] = batch_data.pseudolabels[3]

    return mc_mva,hs_mva,dsr_mva,rbd_mva, image2indx


### update when train
def train_round_update(args, target_dirs, output_dir_it, discretization_threshold):
    ### log
    log_handler = logging.FileHandler(output_dir_it + '/log.txt')
    logger.addHandler(log_handler)
    batch_size = args.batch_size
    ...
    if args.pretrained:
        ...
        for name, param in load_dict.items():
            ...
            if own_dict[name].size() != load_dict[name].size():
                print(
                    'Size of pretrained model and your model does not match in {} ({} vs. {}). Layer stays initialized randomly.' \
    ...
    train_loader = torch.utils.data.DataLoader(
        SegList(args, data_dir, 'train', transforms.Compose(t),
                image_dir=join(args.root_dir, 'Data/01_img/'), gt_dir=join(args.root_dir, 'Data/02_gt/'),
                targets=target_dirs, list_dir=args.data_dir, out_name=True),###,sp_dir=join(args.root_dir, 'Data/sp/')
        batch_size=batch_size, shuffle=True, num_workers=0,  ###num_workers,
        pin_memory=False, drop_last=True
    )
    ...
    val_loader = torch.utils.data.DataLoader(
        SegList(args, data_dir, 'val', transforms.Compose(t_val),
                image_dir=join(args.root_dir, 'Data/01_img/'), gt_dir=join(args.root_dir, 'Data/02_gt/'),
                targets=None, list_dir=args.data_dir, out_name=True),
        batch_size=batch_size, shuffle=False, num_workers=0,  ###num_workers,
        pin_memory=False, drop_last=True
    )
    '''
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            print("=> no checkpoint found at '{}'".format(args.resume))
    ###
    mc_mva,hs_mva,dsr_mva,rbd_mva, image2indx = init_mva_preds_update(args, train_loader)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        trainloss, mc_mva,hs_mva,dsr_mva,rbd_mva = train_update(train_loader,
                                     model,
                                     optimizer,
                                     epoch,
                                     output_dir_it,
        ...
        if (epoch + 1) >=20 and (epoch+1)<=50:#####args.checkpoint_freq
            torch.save({
                'epoch': epoch + 1,
                'arch': args.arch,
                'state_dict': model.state_dict(),
        ...
        if (epoch + 1) >50 and (epoch+1)%5==0:#####args.checkpoint_freq
            torch.save({
                'epoch': epoch + 1,
                'arch': args.arch,
                'state_dict': model.state_dict(),
...
def fus(args):
    ...
    rlabel_directory = join(args.root_dir, 'Fus/')
    os.makedirs(rlabel_directory, exist_ok=True)

    ##### set directory
    rtarget_dirs = []
    rtarget_dirs.append(join(args.root_dir,'refined_nocrf_3/MC/'))
    rtarget_dirs.append(join(args.root_dir,'refined_nocrf_3/HS/'))
    rtarget_dirs.append(join(args.root_dir,'refined_nocrf_3/DSR/'))
    rtarget_dirs.append(join(args.root_dir,'refined_nocrf_3/RBD/'))
    #rtarget_dirs.append(join(args.root_dir,'Doc/Phase_I_Refined_Maps/MC/Iteration_3/MVAMaps/'))
    #rtarget_dirs.append(join(args.root_dir,'Doc/Phase_I_Refined_Maps/HS/Iteration_3/MVAMaps/'))
    #rtarget_dirs.append(join(args.root_dir,'Doc/Phase_I_Refined_Maps/DSR/Iteration_3/MVAMaps/'))
    #rtarget_dirs.append(join(args.root_dir,'Doc/Phase_I_Refined_Maps/RBD/Iteration_3/MVAMaps/'))

    args.epochs = 50
    args.beta = 2####
    args.lr = 1e-4
    train_round_update(args, rtarget_dirs, rlabel_directory, 0.5)



### test for ecssd
def test_saliency_ecssd(args):
    batch_size = args.batch_size
    num_workers = 0  ###args.workers
    test_dir = join(args.root_dir, 'Doc/Test_ecssd/')
    ...
    dataset = SegList_test(args, data_dir, 'ecssd', transforms.Compose([
        transforms.Resize_Image(args.crop_size),
        transforms.ToTensor(),
        normalize,
    ]), image_dir=join(args.root_dir, 'Data_ecssd/img/'), gt_dir=join(args.root_dir, 'Data_ecssd/gt/'),
                           list_dir=args.data_dir, out_name=True)
    test_loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size, shuffle=False, num_workers=0,  ###num_workers,
    ...
    #if not args.resume:
        #####
    args.resume = join(args.root_dir + args.weight_path)####/Doc/Phase_II_Fusion/checkpoint_075.pth.tar  ####/Fus/checkpoint_200.pth.tar
    start_epoch = 0
    if args.resume:
        if os.path.isfile(args.resume):
            logger.info("=> loading checkpoint '{}'".format(args.resume))
...
### test for duts-te
def test_saliency_duts_te(args):
    batch_size = args.batch_size
    num_workers = 0  ###args.workers
    test_dir = join(args.root_dir, 'Doc/Test_duts-te/')
    ...
    dataset = SegList_test(args, data_dir, 'duts-te', transforms.Compose([
        transforms.Resize_Image(args.crop_size),
        transforms.ToTensor(),
        normalize,
    ]), image_dir=join(args.root_dir, 'Data_duts-te/img/'), gt_dir=join(args.root_dir, 'Data_duts-te/gt/'),
                           list_dir=args.data_dir, out_name=True)
    test_loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size, shuffle=False, num_workers=0,  ###num_workers,
...
### test for dut-omron
def test_saliency_dut_omron(args):
    batch_size = args.batch_size
    num_workers = 0  ###args.workers
    test_dir = join(args.root_dir, 'Doc/Test_dut-omron/')
    ...
    dataset = SegList_test(args, data_dir, 'dut-omron', transforms.Compose([
        transforms.Resize_Image(args.crop_size),
        transforms.ToTensor(),
        normalize,
    ]), image_dir=join(args.root_dir, 'Data_dut-omron/img/'), gt_dir=join(args.root_dir, 'Data_dut-omron/gt/'),
                           list_dir=args.data_dir, out_name=True)
    test_loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size, shuffle=False, num_workers=0,  ###num_workers,
...
### test for hku-is
def test_saliency_hku_is(args):
    batch_size = args.batch_size
    num_workers = 0  ###args.workers
    test_dir = join(args.root_dir, 'Doc/Test_hku-is/')
    ...
    dataset = SegList_test(args, data_dir, 'hku-is', transforms.Compose([
        transforms.Resize_Image(args.crop_size),
        transforms.ToTensor(),
        normalize,
    ]), image_dir=join(args.root_dir, 'Data_hku-is/img/'), gt_dir=join(args.root_dir, 'Data_hku-is/gt/'),
                           list_dir=args.data_dir, out_name=True)
    test_loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size, shuffle=False, num_workers=0,  ###num_workers,
...
### test for pascal-s
def test_saliency_pascal_s(args):
    batch_size = args.batch_size
    num_workers = 0  ###args.workers
    test_dir = join(args.root_dir, 'Doc/Test_pascal-s/')
    ...
    dataset = SegList_test(args, data_dir, 'pascal-s', transforms.Compose([
        transforms.Resize_Image(args.crop_size),
        transforms.ToTensor(),
        normalize,
    ]), image_dir=join(args.root_dir, 'Data_pascal-s/img/'), gt_dir=join(args.root_dir, 'Data_pascal-s/gt/'),
                           list_dir=args.data_dir, out_name=True)
    test_loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size, shuffle=False, num_workers=0,  ###num_workers,
    ...
    if args.cmd == 'train':
        ...
        for k in range(0,20):
            ...
            if(len(_tmp)==2):
                _tmp='0'+_tmp    
            print(_tmp)
            args.weight_path='/Fus/checkpoint_'+_tmp+'.pth.tar'
            print(args.weight_path)
        ###/Fus/checkpoint_080.pth.tar
        ###/Doc/Phase_II_Fusion/checkpoint_075.pth.tar
            args.dataset_name='ECSSD'
            test_saliency_ecssd(args)
        
            #args.dataset_name='DUTS-TE'
            #test_saliency_duts_te(args)

            #args.dataset_name='DUT-OMRON'
            #test_saliency_dut_omron(args)

            #args.dataset_name='PASCAL-S'
            #test_saliency_pascal_s(args)
            
            #####
            #args.dataset_name='HKU-IS'
            #test_saliency_hku_is(args)
        

        #### msrab is not tested
        #args.dataset_name='MSRAB'
        #test_saliency(args)
