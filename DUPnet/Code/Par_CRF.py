...
class CRFDataset(torchvision.datasets.DatasetFolder):
    ...
    def __getitem__(self, index):
        ...
        '''
        #####
        sal_pred_dcrf = torch.tensor(sal_pred[0])####apply_dcrf_single(sal_pred,Color,name)####torch.tensor(sal_pred[0])
...
def init_crf_loader(sal_preds, images_names, args, Color=True, crf_on_mva=True):
    ...
    crf_loader = torch.utils.data.DataLoader(
         ...
         shuffle=False,
         num_workers= 0,###min(batch_size,24),
         pin_memory=False,
         drop_last=False)
    return crf_loader
