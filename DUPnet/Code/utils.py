...
from capsules_ import capsules_
...
class DRNSeg(nn.Module):
    def __init__(self, model_name, classes, pretrained_model=None,
        ...
        #### this changged  for update pretrained weight
        if pretrained_model is not None:
            pmodel.load_state_dict(pretrained_model)

        self.base = nn.Sequential(*list(pmodel.children()))
        ...
        if use_torch_up:
            ...
            up = nn.ConvTranspose2d(4, classes, 16, stride=8, padding=4,#####
 ...
 ###
        ...
        model_capsule_1= capsules_(A=8,B=8,C=8,D=8,E=10,iters=2)
        model_capsule_1= nn.DataParallel(model_capsule_1)
        self.capsule_1 =nn.Sequential(*list(model_capsule_1.children()))


        self.seg_cap_1 = nn.Conv2d(8, 2,
                             kernel_size=1, bias=True)

        m_cap_1 = self.seg_cap_1
        n_cap_1 = m_cap_1.kernel_size[0] * m_cap_1.kernel_size[1] * m_cap_1.out_channels
        m_cap_1.weight.data.normal_(0, math.sqrt(2. / n_cap_1))
        m_cap_1.bias.data.zero_()


        up_cap_1 = nn.ConvTranspose2d(2, 2, 2, stride=2, padding=0,
                                    output_padding=0, groups=1,
                                    bias=False)
        fill_up_weights(up_cap_1)
        up_cap_1.weight.requires_grad = False
        self.up_cap_1 = up_cap_1

        self.weight_cap_1=nn.Conv2d(128,1,kernel_size=44,bias=False)


        ###

        model_capsule_2= capsules_(A=8,B=8,C=8,D=8,E=10,iters=2)
        model_capsule_2= nn.DataParallel(model_capsule_2)
        self.capsule_2 =nn.Sequential(*list(model_capsule_2.children()))


        self.seg_cap_2 = nn.Conv2d(8, 2,
                             kernel_size=1, bias=True)

        m_cap_2 = self.seg_cap_2
        n_cap_2 = m_cap_2.kernel_size[0] * m_cap_2.kernel_size[1] * m_cap_2.out_channels
        m_cap_2.weight.data.normal_(0, math.sqrt(2. / n_cap_2))
        m_cap_2.bias.data.zero_()


        up_cap_2 = nn.ConvTranspose2d(2, 2, 2, stride=2, padding=0,
                                    output_padding=0, groups=1,
                                    bias=False)
        fill_up_weights(up_cap_2)
        up_cap_2.weight.requires_grad = False
        self.up_cap_2 = up_cap_2

        self.weight_cap_2=nn.Conv2d(128,1,kernel_size=44,bias=False)


        ###

        model_capsule_3= capsules_(A=8,B=8,C=8,D=8,E=10,iters=2)
        model_capsule_3= nn.DataParallel(model_capsule_3)
        self.capsule_3 =nn.Sequential(*list(model_capsule_3.children()))


        self.seg_cap_3 = nn.Conv2d(8, 2,
                             kernel_size=1, bias=True)

        m_cap_3 = self.seg_cap_3
        n_cap_3 = m_cap_3.kernel_size[0] * m_cap_3.kernel_size[1] * m_cap_3.out_channels
        m_cap_3.weight.data.normal_(0, math.sqrt(2. / n_cap_3))
        m_cap_3.bias.data.zero_()


        up_cap_3 = nn.ConvTranspose2d(2, 2, 2, stride=2, padding=0,
                                    output_padding=0, groups=1,
                                    bias=False)
        fill_up_weights(up_cap_3)
        up_cap_3.weight.requires_grad = False
        self.up_cap_3 = up_cap_3

        self.weight_cap_3=nn.Conv2d(128,1,kernel_size=44,bias=False)


        ###

        model_capsule_4= capsules_(A=8,B=8,C=8,D=8,E=10,iters=2)
        model_capsule_4= nn.DataParallel(model_capsule_4)
        self.capsule_4 =nn.Sequential(*list(model_capsule_4.children()))


        self.seg_cap_4 = nn.Conv2d(8, 2,
                             kernel_size=1, bias=True)

        m_cap_4 = self.seg_cap_4
        n_cap_4 = m_cap_4.kernel_size[0] * m_cap_4.kernel_size[1] * m_cap_4.out_channels
        m_cap_4.weight.data.normal_(0, math.sqrt(2. / n_cap_4))
        m_cap_4.bias.data.zero_()


        up_cap_4 = nn.ConvTranspose2d(2, 2, 2, stride=2, padding=0,
                                    output_padding=0, groups=1,
                                    bias=False)
        fill_up_weights(up_cap_4)
        up_cap_4.weight.requires_grad = False
        self.up_cap_4 = up_cap_4

        self.weight_cap_4=nn.Conv2d(128,1,kernel_size=44,bias=False)      
    ...
    def forward(self, x):
        ...
        x1=x[:,0:128,:,:]
        x1_=self.weight_cap_1(x1)    
        x1=self.capsule_1(x1)
        x1=x1.permute(0,3,1,2)
        x1=self.seg_cap_1(x1)
        x1=self.up_cap_1(x1)
        x1=x1*x1_
        #print(x1.shape)
        #print(x1.device)
        ...
        x2=x[:,128:256,:,:]
        x2_=self.weight_cap_2(x2)  
        x2=self.capsule_2(x2)
        x2=x2.permute(0,3,1,2)
        x2=self.seg_cap_2(x2)
        x2=self.up_cap_2(x2)
        x2=x2*x2_
        #print(x2.shape)
        #print(x2.device)
        ...
        x3=x[:,256:384,:,:]
        x3_=self.weight_cap_3(x3)  
        x3=self.capsule_3(x3)
        x3=x3.permute(0,3,1,2)
        x3=self.seg_cap_3(x3)
        x3=self.up_cap_3(x3)
        x3=x3*x3_
        #print(x3.shape)
        #print(x3.device)
        ...
        x4=x[:,384:512,:,:]
        x4_=self.weight_cap_4(x4)  
        x4=self.capsule_4(x4)
        x4=x4.permute(0,3,1,2)
        x4=self.seg_cap_4(x4)
        x4=self.up_cap_4(x4)
        x4=x4*x4_
        #print(x4.shape)
        #print(x4.device)
...
def F_cont(sal_pred, Disc_Label, b=1.5): #### some revise for prec and recall
    ...
    if int(torch.__version__[0])>0:
        ...
        FN=torch.sum(torch.sum(FN, dim=2), dim=1)
    eps=1e-5
    prec=TP/(TP+FP+eps)
    recall=TP/(TP+FN+eps)
...
def parse_args():
    ...
    parser.add_argument('cmd', choices=['train', 'test', 'test_all','fus'])###
