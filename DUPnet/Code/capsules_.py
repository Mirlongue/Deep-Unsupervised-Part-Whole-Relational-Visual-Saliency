...
class ConvCaps(nn.Module):
    ...
    def add_pathes(self, x, B, K, psize, stride):
        ...
        '''
        b, h, w, c = x.shape
        tmp = torch.cuda.FloatTensor(b, h, 1, c).fill_(0.)
        x = torch.cat([tmp, x], dim=2)
        x = torch.cat([x, tmp], dim=2)
        tmp = torch.cuda.FloatTensor(b, 1, w+2, c).fill_(0.)
...
class CapsNet(nn.Module):
    ...
    def __init__(self, A=32, B=32, C=32, D=32, E=10, K=3, P=4, iters=3):
        ...
        self.conv1 = nn.Conv2d(in_channels=128, out_channels=A,#####
                               kernel_size=5, stride=2, padding=2)#####
        self.bn1 = nn.BatchNorm2d(num_features=A, eps=0.001,
                                 momentum=0.1, affine=True)
        self.relu1 = nn.ReLU(inplace=False)
        self.primary_caps = PrimaryCaps(A, B, 1, P, stride=1)
        self.conv_caps1 = ConvCaps(B, C, K, P, stride=1, iters=iters)###原步长是2
